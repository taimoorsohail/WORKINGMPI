...
!       allocate (NU_T_mean(0:NZ+1,1:NY+1))         
       ...
       do j=0,NY+1
        do k=0,NZ+1
         ...
         Sbar_MEAN(k,j)=SUM(S2X(0:min(NXP,NXP_L),k,j))/dble(NX)
        end do
       end do

       CALL MPI_COMBINE_STATS(NU_T_MEAN,NZ+2,NY+2)
       CALL MPI_COMBINE_STATS(Sbar_MEAN,NZ+2,NY+2)
...
!     To get S_ij for strain rate tensor we need to store tau_ij 
      ...
      DO ij = 1,6
        ...
        do j=1,NY
         do k=1,NZ
          do i=0,NXP
            Sij(i,k,j,ij) = -2.d0*C_DYN(k,j)*DELTA_Y(k,j)**2.d0*S2X(i,k,j)*st_rij(i,k,j,ij)
          enddo
         enddo
        enddo
...
! du2/dy => 
            ...
            CSij(I,K,J,2) =  (CU2X(I,K,J+1)-CU2X(I,K,J))/DYF(J) 

! du3/dz => 
            CSij(I,K,J,3) =  (CU3X(I,K+1,J)-CU3X(I,K,J))/DZF(K)

!  1/2(du1/dy+du2/dx) =>
            CSij(I,K,J,4) =  0.50d0*((CU1X(I,K,J+1) - CU1X(I,K,J-1)) /(2.0d0*DYF(J))    &
                            + 0.50d0*CIKXP(I)*(CU2X(I,K,J+1) + CU2X(I,K,J)) )

!  1/2(du1/dz+du3/dx) =>
            CSij(I,K,J,5) =  0.50d0*((CU1X(I,K+1,J) - CU1X(I,K-1,J)) / (2.0d0*DZF(K))   &
                            + 0.50d0*CIKXP(I)*(CU3X(I,K+1,J)+CU3X(I,K,J)) )
    
!  1/2(du2/dz+du3/dy) =>
            CSij(I,K,J,6) =  0.5d0*( 0.5*(CU2X(I,K+1,J+1) + CU2X(I,K+1,J)           &
                              - CU2X(I,K-1,J+1) - CU2X(I,K-1,J) )/(2.0d0*DZF(K)) &
                            +  0.5*(CU3X(I,K+1,J+1) + CU3X(I,K,J+1)              &
                              - CU3X(I,K+1,J-1) - CU3X(I,K,J-1) )/(2.0d0*DYF(J)) )
...
! CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
subroutine plot_les_para(file_name)
