...
implicit none    
   ...
   real(r8),dimension(0:Z_SIZE*Y_SIZE-1,1:NP) :: STAT_TMP
     ...
     send_rank = 0
     CALL MPI_GATHER( STAT,Z_SIZE*Y_SIZE,MPI_DOUBLE_PRECISION,STAT_TMP,Z_SIZE*Y_SIZE, &
...
!----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      SUBROUTINE MPI_COMBINE_3D(STAT,temp,SIZE_1,SIZE_2,SIZE_3)
!----*|--.---------.---------.---------.---------.---------.---------.-|-------|
use ntypes
use Domain
use Grid
use   mpi_var
implicit none
   integer :: I, J, K, SIZE_1, SIZE_2, SIZE_3, send_rank,en,st
   real(r8),dimension(1:SIZE_1,1:SIZE_2,1:SIZE_3)      :: STAT
   real(r8),dimension(1:SIZE_1,1:SIZE_2,1:SIZE_3,1:NP) :: STAT_TMP
   real(r8),dimension(1:SIZE_1*NP,1:SIZE_2,1:SIZE_3)      :: temp


     send_rank = 0
     CALL MPI_GATHER(STAT,SIZE_1*SIZE_2*SIZE_3,MPI_DOUBLE_PRECISION,STAT_TMP,SIZE_1*SIZE_2*SIZE_3,MPI_DOUBLE_PRECISION,send_rank,MPI_COMM_WORLD,IERROR)
        WRITE(6,*) "I am here one"
...
!----*|--.---------.---------.---------.---------.---------.---------.-|-------|
!     SUBROUTINE MPI_COMBINE_3D(STAT,temp_ZX,Z_SIZE,Y_SIZE)
!----*|--.---------.---------.---------.---------.---------.---------.-|-------|
!use ntypes
!use Domain
!use Grid
!use   mpi_var
!implicit none
!   integer :: I, J, Z_SIZE,Y_SIZE,send_rank,en,st
!   real(r8),dimension(1:Z_SIZE,1:Y_SIZE)      :: STAT
!   real(r8),dimension(1:Z_SIZE,1:Y_SIZE,1:NP) :: STAT_TMP
!   real(r8),dimension(1:Z_SIZE*NP,1:Y_SIZE)      :: temp_ZX
...
implicit none
      ...
      REAL(8),DIMENSION(1:(NXP+1)*NP) :: IPACK
      REAL(8),DIMENSION(1:NXP+1) :: OPACK

! READ IN LOCAL DT BASED ON CFL
!      OPACK = DT

! SEND ALL LOCAL DT'S TO EVERY PROCESS
      CALL MPI_ALLGATHER(OPACK,NXP+1,MPI_DOUBLE_PRECISION,IPACK,NXP+1, &
