...
use mpi_var
use forcing
use variable_stat, only : tke_mean, tke_mean_old, energy_mean, energy_mean_old, &
                          back_potential, back_potential_old
...
allocate (GXF(0:NX+1), stat=s_1)
allocate (GYF(0:NY+1), stat=s_1) 
allocate (GZF(0:NZ+1), stat=s_1)
allocate (DXF(0:NX+1), stat=s_1)
allocate (DYF(0:NY+1), stat=s_1)
allocate (DZF(0:NZ+1), stat=s_1)

allocate (xpoint(0:NZ+1,0:NY+1), ypoint(0:NZ+1,0:NY+1), stat=s_1 )
...
allocate (SINK_PROF(0:NZ+1,0:NY+1), stat=s_1)
allocate (H_hill(0:NXP), stat=s_1)
allocate (H_hill_tot(0:((NXP+1)*NP-1)), stat=s_1)

allocate (hill_ind(0:NXP), stat=s_1)
allocate (hill_ind_tot(0:((NXP+1)*NP-1)), stat=s_1)
...
allocate (THBAR(0:NY+1,1:N_TH+1), stat=s_1)
allocate (temp_mean(0:NZV-1,0:NY+1), stat=s_1) 
allocate (tke_mean(0:NZ+1,0:NY+1), stat=s_1) 
allocate (tke_mean_old(0:NZ+1,0:NY+1), stat=s_1)
allocate (energy_mean(0:NZ+1,0:NY+1), stat=s_1)
allocate (energy_mean_old(0:NZ+1,0:NY+1), stat=s_1)
allocate (back_potential(0:NZ+1,0:NY+1), stat=s_1)
allocate (back_potential_old(0:NZ+1,0:NY+1), stat=s_1)
...
allocate     (MATL (0:NXP,0:NY+1), MATD(0:NXP,0:NY+1), stat=s_1)
allocate     (MATU(0:NXP,0:NY+1), VEC(0:NXP,0:NY+1), stat=s_1)
allocate     (MATL_Z (0:NXP,0:NZ+1), MATD_Z(0:NXP,0:NZ+1), stat=s_1)
allocate     (MATU_Z(0:NXP,0:NZ+1), VEC_Z(0:NXP,0:NZ+1), stat=s_1)
...
! for serial_version of diablo only

!  allocate ( V(1:NY*NZ,0:NX/3), stat=s_1 ) 
!  allocate ( VC(1:NM,0:NX/3),  stat=s_1 )
!  allocate ( VB(1:NY*NZ,0:NX/3), stat=s_1 )
!  allocate ( VBC(1:NM,0:NX/3),  stat=s_1 ) 
!  allocate ( RHS(1:NY*NZ,0:NX/3), stat=s_1 )
!  allocate ( RHSC(1:NM,0:NX/3),  stat=s_1 )
!  allocate ( A(1:NY*NZ*9,0:NX/3),  stat=s_1 )
!  allocate ( AC(1:9*NM,0:NX/3) , stat=s_1)
!  allocate ( LDU(1:3*NY*NZ,0:NX/3), stat=s_1 )
!  allocate ( LDUC(1:3*NM,0:NX/3), stat=s_1 )
!  allocate ( WORK(1:NZ*12,0:NX/3),  stat=s_1 )
!  allocate ( WA(1:NY*NZ,0:NX/3), stat=s_1 )
!  allocate ( WAC(NM,0:NX/3), stat=s_1 )
!  allocate ( WB(1:NY*NZ,0:NX/3), stat=s_1 ) 
!  allocate ( WBC(1:NM,0:NX/3), stat=s_1 )
...
if (s_1.NE.0) then
  write(I_OUT,*) "Error deallocating CFTHX"
endif
...
if (s_1.NE.0) then
  write(I_OUT,*) "Error deallocating FTHX"
endif
...
! &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
allocate     (PE_DISS(0:NY+1,1:N_TH), stat=s_1)
...
allocate     (phi_b2(0:NZ+1), stat=s_1)
if (s_1.NE.0) then
...
allocate     (epsilon_rho(0:NZ+1,0:NY+1), stat=s_1)
!allocate     (tke_mean_old(0:NZ+1,0:NY+1), stat=s_1)
!allocate     (energy_mean_old(0:NZ+1,0:NY+1), stat=s_1)
! &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
! allocate tke variavles 
      ...
      allocate (Z_star_bar(0:NZ+1,0:NY+1), stat=s_1)
if (s_1.NE.0) then
...
! deallocate variables needed for SAVE_STATS_TH
      ...
      deallocate (PE_DISS, Rig)
      deallocate (THRMS,th_wh,THV, THW, DTHDY, DTHDZ, DELTA_N2)
...
allocate     (Sbar_MEAN(0:NZ+1,1:NY+1), stat=s_1)
