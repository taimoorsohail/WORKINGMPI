...
! Absorb -2.d0*C_SMAG**2.d0 here for effienciency
      ...
      DO J=0,NY+1
       DO K=0,NZ+1
! At GY points:
        DELTA_Y(K,J)=-C_SMAG**2.d0*(beta*DX(1)*DYF(J)*DZF(K))**(2.d0/3.d0)
...
! temp=Ui*Uj:
       ...
       CASE(2)
         do j=0,NY+1
           do k=0,NZ+1
             do i=0,NXP
               temp(i,k,j)=THX(i,k,j,n)*0.50*(U2X(i,k,j+1)+U2X(i,k,j))
             end do
           end do
         end do
       CASE(3)
         do j=0,NY+1
           do k=0,NZ+1
             do i=0,NXP
               temp(i,k,j)=THX(i,k,j,n)*0.50*(U3X(i,k+1,j)+U3X(i,k,j))
             end do
           end do 
         end do
       END SELECT
...
! If C_DYN_TH < 0 at any level, set C_DYN_TH=0 for numerical stability
      do j=0,NY+1
       do k=0,NZ+1
        ...
        C_DYN_TH(k,j,N) = C_DYN_TH(k,j,N)*damp_fact(k)
       end do
      end do

       do j=0,NY+1
       do k=0,NZ+1
         CALL MPI_BCAST_REAL(C_DYN_TH(k,j,n),1,1)
       end do
       end do
      ...
      DO J=JSTART_TH(N),JEND_TH(N) 
       DO K=ZSTART_TH(N),ZEND_TH(N)
          DO I=0,NX2P
            ...
            CSij(I,K,J,2)=(CTHX(I,K,J+1,N) - CTHX(I,K,J-1,N)) /(2.0d0*DYF(J))
            CSij(I,K,J,3)=(CTHX(I,K+1,J,N) - CTHX(I,K-1,J,N)) /(2.0d0*DZF(K))
          END DO
        END DO
      END DO
