...
!      INCLUDE 'header_fft'
      ...
      parameter (ni=NX, nj=NY+2,nk_st=690,nk=2000,NP=257)
...
!     &       th_wh(ni,nj)
 ...
 !      real(r4),allocatable,dimension(:) :: g1vtk,g2vtk
       ...
       real(8) :: sum_1, sum_2, prof_1(nj), prof_2(nj)
...
!       PARAMETER (NX2P=NX2V/NP-1)    
	...
	DO k=nk_st,nk
	 ...
	 DO rank=0,NP-1
       ...
       file_name_1 = 'plane_data_XY/'//base_name//file_num//PID//'.pln'
...
!       t(kk) = time
       ...
       do j=1,nj
        ...
        enddo
         prof_1(j) = sum_1/ni
         prof_2(j) = sum_2/ni
         write(202,*) y(1,j), prof_1(j), prof_2(j) 
       enddo  
         write(203,*) time, prof_1(nj), prof_2(nj)
...
!Field
        ...
        ii = ii+1
         DO N=1,N_TH
         th_wh(i,j)= real(th(i,j)) + 35.0d0 !+  THBAR(j,N) !
         ENDDO
        enddo
        enddo
