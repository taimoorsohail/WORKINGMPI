...
module Domain
 ...
 integer(i4)       :: nx, ny, nz, N_TH, NXM, NYM, NZM, TNKZ, TNKY, ipn, jpn1, jpn2, kpn1, kpn2
 integer(i4)       :: NKX
...
!----*|--.---------.---------.---------.---------.---------.---------.-|-------|
      LOGICAL  ::USE_MPI,CONT_STRAT, IBM, SPONGE_NORTH, SPONGE_SOUTH, SPONGE_EAST,SPONGE_WEST
      ...
      real(r8),allocatable,dimension(:,:)    ::   SPONGE_SIGMA,SPONGE_SIGMA_OUT, &
                                                  SPONGE_TEMP, SINK_PROF  
      real(r8),allocatable,dimension(:)      ::   C_int, C_int_le, H_hill, H_hill_tot
      integer, allocatable,dimension(:)      ::   hill_ind, hill_ind_tot
      ...
      real(r8) :: RI_TAU(1:N_TH), PR(1:N_TH), FACT_AMP,Ratio_gr,Gravity,rho_0, &
                  alpha_T,gamma_w,Ratio_gr_a,Ratio_gr_g, Sal_0, theta_0, dSaldz, &
                  TH_1, TH_2, Loc_s, Lh_s, f_0, beta_f 
      logical  :: CREATE_NEW_TH(1:N_TH), BACKGROUND_GRAD(1:N_TH),DEV_BACK_TH
      integer  :: NUM_READ_TH
      ...
      integer  :: JSTART_TH(1:N_TH),JEND_TH(1:N_TH),ZSTART_TH(1:N_TH), &
                  ZEND_TH(1:N_TH) 
      real(r8) :: OMEGA0, AMP_OMEGA0, ANG_BETA,In_H0,Q_H0,H0
      ...
      real(r8),allocatable,dimension(:,:) ::    THBAR 
...
!      real(r8) ::  U1 (0:NX+1,0:NZ+1,0:NY+1)
!      real(r8) ::  U2 (0:NX+1,0:NZ+1,0:NY+1)
!      real(r8) ::  U3 (0:NX+1,0:NZ+1,0:NY+1)
!      real(r8) ::  P  (0:NX+1,0:NZ+1,0:NY+1)
!      real(r8) ::  TH (0:NX+1,0:NZ+1,0:NY+1,1:N_TH)
! 
...
! !    ARRAYS FOR WHEN DATA IS SPLITED IN X DIRECTION
! 
!      complex(r8) :: CU1(0:NX/2,0:NZ+1,0:NY+1)
!      complex(r8) :: CU2(0:NX/2,0:NZ+1,0:NY+1)
!      complex(r8) :: CU3(0:NX/2,0:NZ+1,0:NY+1)
!      complex(r8) :: CP (0:NX/2,0:NZ+1,0:NY+1)
!      complex(r8) :: CTH(0:NX/2,0:NZ+1,0:NY+1,1:N_TH)
!   
...
!          MATU_Z(0:NX-1,0:NZ+1), VEC_Z(0:NX-1,0:NZ+1)
      ...
      real(r8) :: TIME_OLD,TIME_OLD_ENG,TIME_OLD_POT
...
!!$omp threadprivate(/THOMAS_TH_Z/, /THOMAS_TH/)
      REAL(r8),allocatable,DIMENSION(:,:) :: MATL,MATD,MATU,VEC
      REAL(r8),allocatable,DIMENSION(:,:) :: MATL_Z,MATD_Z,MATU_Z,VEC_Z
...
! Variables for outputting statistics
     ...
     real(r8) ::     URMS_B,VRMS_B,WRMS_B,TKE_B, area

! Variables needed for SAVE_STATS_TH
     
      real(r8),allocatable,dimension(:,:) :: PE_DISS, Rig
...
! Variables for tkebudget
      ...
      real(r8) :: sum_Prod, sum_dissp, sum_buoyF,     &  
      sum_dtkdt, sum_vis, sum_trans, sum_advec,sum_dissp_sgs,sum_prod_sgs
      
      real(r8),allocatable,dimension(:,:) :: Z_star_bar 
...
use Domain
    ...
    REAL(r8) P_TMP(NZ,NY), P_TMP2(NZ,NY), RHS_TMP(NZ,NY)
...
! 
       real(r8),allocatable,dimension(:,:) :: NU_T_mean, DELTA_Y,U1_bar_les, &
                                              U3_bar_les,U2_bar_les,C_DYN, Sbar_MEAN
